init -10 python:
    ...
    class AttackSkill(Skill):
        ...
        def __init__(self, multiplier=1, hitt=100, damage=0, range=1, sfx=None, endTurn=True, name="Attack", command=None, attributes=["melee"], hotkey='a', weight=0):
        ...
        def SetUpFighter(self, fighter):
            ...
            fighter.RegisterStat("Hitt", 0)
            fighter.RegisterStat("Gritt", 0)
        ...
        def PerformAction(self, fighter, target):
            if (target != None):
                ...
                if 100 <= (renpy.random.random()*100)+ (self._hitt + fighter.Stats.Hitt+ (target[0].Stats.Chain*2))- ((target[0].Position.Height*10)+target[0].Stats.Gritt):
                    if ('Speech' in self._attributes):
                        fighter._battle.Attack(fighter, ((fighter.Stats.Speech* self._multiplier)+ self._damage -target[0].Stats.Mind), self._attributes, target[0], target[1])
                    if ('Magic' in self._attributes):
                        fighter._battle.Attack(fighter, (((fighter.BaseStats.MP + fighter.Stats.MP)* self._multiplier)+ self._damage, -target[0].Stats.MP), self._attributes, target[0], target[1])
                    else:
                        fighter._battle.Attack(fighter, ((fighter.Stats.Attack* self._multiplier)+ self._damage -target[0].Stats.Defence), self._attributes, target[0], target[1])
                else:
                    fighter._battle.Announce(fighter.Name + " Fumbled")
                fighter._battle.ChangeFighterState(fighter, "default")
                target[0].Stats.Chain = target[0].Stats.Chain + fighter.Stats.Suport
                fighter.Stats.Chain = 0
    ...
    class MoveSkill(Skill):
        ...
        def SetUpFighter(self, fighter):
            ...
            fighter.RegisterStat("Bunus_Move", 1)
            #fighter.Stats.Bonus_Move
        def GetTargets(self, fighter):
                        ...
                        #            dh = pos3.Height - pos.Height
                         ...
                         #           # or .5 lower than the space we're moving from - and it costs two movement + diff.
                          #          if dh <= 0.25 and dh >= -0.5:
                           ...
                           #             joins[pos3] = battlefield.GetRange(pos, pos3, useHeight=False) + 0.0
        ...
        def FighterStartTurn(self, fighter):
            #fighter.Stats.Move = fighter.RawStats.Move
            fighter.Stats.Move = fighter.BaseStats.Move + fighter.Stats.Bonus_Move
    
    class MagicFighterAttackSkill(Skill):
        
        def __init__(self, name="Magic Fighter Attack", command=None, attributes=[''], damage=1, multiplier=1, hitt=100, cost=1, range=5, sprite=None, pause=0, sfx=None, endTurn=True, hotkey=None, weight=0):
            self._damage = damage
            self._multiplier = multiplier
            self._hitt = hitt
            self._cost = cost
            ...
            if (self.IsAvailable(fighter)):
                if (target != None):
                    ...
                    if 100 <= (renpy.random.random()*100)+ (self._hitt + fighter.Stats.Hitt+ (target[0].Stats.Chain*2))- ((target[0].Position.Height*10)+target[0].Stats.Gritt):
                        if ('Speech' in self._attributes):
                            fighter._battle.Attack(fighter, ((fighter.Stats.Speech*self._multiplier)+ self._damage - target[0].Stats.Mind), self._attributes, target[0], target[1])
                        elif ('Magic' in self._attributes):
                            fighter._battle.Attack(fighter, (((fighter.BaseStats.MP + fighter.Stats.MP)*self._multiplier)+ self._damage -target[0].Stats.MP), self._attributes, target[0], target[1])
                        else:
                            fighter._battle.Attack(fighter, ((fighter.Stats.Attack *self._multiplier) + self._damage - target[0].Stats.Defence), self._attributes, target[0], target[1])
                    else:
    ...
    class FriendlyAttackSkill(Skill):
        
        def __init__(self, name="Friendly Fighter Attack", command=None, attributes=['Friendly'], damage=1, multiplier=1, hitt=100, cost=1, range=1, sprite=None, pause=0, sfx=None, endTurn=True, hotkey=None, weight=0):
            self._damage = damage
            self._multiplier = multiplier
            self._hitt = hitt
            self._cost = cost
            self._targets = TargetData(fighters=True, positions=False, factions=False, los=True, friendly=True, enemy=True, live=True, dead=False, range=range, fightersImpede=True)
            self._sprite = sprite
            self._pause = pause
            self._sfx = sfx
            self._endTurn = endTurn
            ...
            super(FriendlyAttackSkill, self).__init__(name=name, command=command, hotkey=hotkey, weight=weight)


            #self._targets = TargetData(fighters=True, positions=False, factions=False, los=True, friendly=True, enemy=True, live=False, dead=True, range=range, fightersImpede=True)
        def SetUpFighter(self, fighter):
            fighter.RegisterStat("MP", 10)
            fighter.RegisterStat("Magic", 10)
                ...
                if (target != None):
                    ...
                    if self._sprite != None and self._pause > 0:
                        ...
                        if (_preferences.battle_skip_combat == False):
                            _battle.Pause(self._pause)
                        renpy.hide(tag, layer=l)
                    if target[0].Faction == fighter.Faction:
                        damage = (((fighter.Stats.Suport+ target[0].Stats.Suport)/2)*self._multiplier)+self._damage
                        damage = damage *-1
                        target[0].Damage(damage, fighter)
                    else:    
                     if 100 <= (renpy.random.random()*100)+ (self._hitt + fighter.Stats.Hitt+ (target[0].Stats.Chain*2))- ((target[0].Position.Height*10)+target[0].Stats.Gritt):
                        if ('Speech' in self._attributes):
                            fighter._battle.Attack(fighter, ((fighter.Stats.Speech*self._multiplier)+ self._damage -target[0].Stats.Mind), self._attributes, target[0], target[1])
